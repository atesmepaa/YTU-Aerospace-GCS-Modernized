...
SiK telemetri radyosu üzerinden JSON haberleşmesini yönetir.
  - Otomatik yeniden bağlanma (reconnect_loop)
  - Ping (ping_loop)
  - Telemetri sağlığı izleme (ui_link_indicator_loop)
  - Gelen mesajlar on_message / on_link_status callback'leriyle iletilir
"""
...
class SiKLink:
    def __init__(self, on_message, on_link_status):
        """
        on_message(msg: dict)      — UI thread'de çağrılmalı (after() ile sar)
        on_link_status(s: str)     — "BEKLENİYOR" | "BAĞLI" | "ZAYIF" | "KOPUK"
        """
    ...
    def _try_open(self):
        try:
            new_sik = serial.Serial(SIK_PORT, SIK_BAUD, timeout=0.1)
            with self._lock:
                self._sik = new_sik
    ...
    def _close_sik_locked(self):
        """Lock zaten alınmışken portu kapat."""
        try:
            self._sik.close()
        except Exception:
            pass
                ...
                if chunk:
                    ...
                    while b"\n" in buf:
                        ...
                        try:
                            ...
                            if DEBUG_JSON_FAIL:
                                print(f"[SiK] JSON parse fail: {e} | raw={line[:120]}")
            except Exception:
                with self._lock:
                    if self._sik is sik:
                        self._close_sik_locked()
    ...
    def _link_indicator_loop(self):
        """pc_link gelmese bile telemetri akışından bağlantı durumu çıkar."""
        while True:
            now = time.time()
